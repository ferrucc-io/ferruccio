...
function startWizard()
{
    ...
    do {
        nbParties = prompt('How many sections? (1-8)');
    } while (!isInt(nbParties) || nbParties < 1 || nbParties > 8);
    nbParties = parseInt(nbParties);

    withMenu = true;

    const menuName = prompt('Menu title (Topic) ?', 'Metallurgy');
    courseHeader += `\nLbl 0\nMenu("${menuName.toUpperCase()}"`;

    for (var i = 0; i < nbParties; i++)
    {
        parties[i] = prompt(`Title of section n°${(i+1)}`);
        courseHeader += `,"${parties[i].toUpperCase()}",${(i+1)}`;
        $('.step2').fadeIn(1000).append(`<textarea id="myText${i}" title="Your text goes here" name="myText${i}" placeholder="You can write your text here..">Your topic section ${(i+1)}! </textarea><br />`);
    }
...
function create8xpFile(title, content)
{
    try
    {
        ...
        if (file)
        {
            ...
            alert('Can\'t get the file created:(');
        }
